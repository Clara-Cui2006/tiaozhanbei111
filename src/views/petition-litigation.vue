...
const ItemTable = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () => h('section', { class: 'tech-panel table-panel' }, [h('div', { class: 'panel-title' }, [h('span', props.reverse ? '反向审视事项' : '全量事项列表'), h('small', `共 ${props.items.length} 条`)]), h(resolveComponent('a-table') as any, { columns: columns.value, data: props.items, pagination: { pageSize: 8 }, rowKey: 'id', scroll: { x: 1000, y: focusedPanel.value.includes('items') ? 420 : 395 } }, { riskLevel: ({ record }: any) => h('span', { class: 'table-risk-light', style: { display: 'inline-block', width: '18px', height: '18px', borderRadius: '50%', backgroundColor: riskColor(record.riskLevel), border: `2px solid ${riskColor(record.riskLevel)}`, boxShadow: `0 0 8px ${riskColor(record.riskLevel)}, 0 0 18px ${riskColor(record.riskLevel)}` }, title: record.riskLevel, 'aria-label': `${record.riskLevel}风险` }), action: ({ record }: any) => h(resolveComponent('a-button') as any, { type: 'text', size: 'small', onClick: () => emit('open', record) }, () => '查看详情') })])
  }
})
...
.table-risk-light { display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(255,255,255,.45); border-radius: 50%; box-shadow: 0 0 12px currentColor; }
</style>
