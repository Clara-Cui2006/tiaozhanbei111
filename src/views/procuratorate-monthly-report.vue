          ...
          <article class="report-paper">
            ...
            <div class="report-grid">
              ...
              <section :id="'section-keyGroups'" class="report-card group-card">
                ...
                <div v-else-if="report.metrics.groups.length" class="donut-layout"><div class="donut" :style="donutStyle"></div><ul><li v-for="(item,index) in report.metrics.groups" :key="item.name"><i :style="{ background: groupColor(item.name, index) }"></i>{{ item.name }} <b>{{ item.percentage }}%</b></li></ul></div>
...
const groupColors = ['#1465ca', '#4e93e6', '#8bb8ee', '#d2aa5a', '#8465c5', '#36a79a', '#e58a6b']
const groupColor = (name: string, index: number) => name === '非特定重点群体' ? '#9aa8b8' : groupColors[index % groupColors.length]
...
const donutStyle = computed(() => { let start = 0; const segments = (report.value?.metrics.groups || []).map((item,index) => { const end = start + item.percentage; const value = `${groupColor(item.name, index)} ${start}% ${end}%`; start = end; return value }); segments.push(`#edf3f9 ${Math.min(start, 100)}% 100%`); return { background: `conic-gradient(${segments.join(',')})` } })
...
.donut-layout{min-height:250px;gap:48px;padding:18px 28px}.donut{width:190px;height:190px;flex:0 0 190px}.donut:after{inset:46px}
